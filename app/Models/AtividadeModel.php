...
class AtividadeModel extends NajModel {
   ...
   public function addAllColumns() {
      $this->addRawColumn("DATE_FORMAT(A.DATA,'%d/%m/%Y') AS DATA_INICIO")
         ...
         ->addRawColumn("TIME_FORMAT(A.HORA_INICIO,'%H:%m:%s') AS HORA_INICIO")
         ->addRawColumn("TIME_FORMAT(A.HORA_TERMINO,'%H:%m:%s') AS HORA_TERMINO")
         ->addRawColumn("TIME_FORMAT(A.TEMPO,'%H:%m:%s') AS TEMPO")
